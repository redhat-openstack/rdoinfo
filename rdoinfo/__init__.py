...
def parse_info_file(fn, apply_tag=None, add_fns=['deps.yml']):
    info = yaml.load(open(fn, 'rb'))
    for add_fn in add_fns:
        add_file = path.join(path.dirname(fn), add_fn)
        add_info = yaml.load(open(add_file, 'rb'))
        info = include_packages(info, add_info)
